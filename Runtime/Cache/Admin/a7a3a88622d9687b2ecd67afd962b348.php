    ...
    <form method='post' action='<?php echo U(' posUpdate ');?>'>
        <article class="module width_full">
            <header>
                ...
                <ul class="tabs">
                    <li><a href="<?php echo U('tplMsgAdd');?>"><i class='icon-plus'></i> 添加新条目</a></li>
                </ul>
            </header>
                            ...
                            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input name='id[]' type='hidden' value='<?php echo ($vo["id"]); ?>'>
                                <tr>
                                    ...
                                    <td class='text-center'>
                                        <a class='my-icon' href='<?php echo U('tplMsgEdit','id='.$vo['id']);?>' title="编辑"><i class='icon-pencil'></i></a>
                                        <a class='my-icon' href='javascript:sure("<?php echo U('tplMsgDel','id='.$vo['id']);?>")' title="删除"><i class='icon-trash'></i></a>
                                    </td>
