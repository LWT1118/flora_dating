...
<section id="main" class="column">
	<form method='post' action='<?php echo U('posUpdate');?>'>
		...
		<ul class="tabs">
   			<li><a href="<?php echo U('add','cat='.$pid);?>"><i class='icon-plus'></i> 添加新条目</a></li>
		</ul>
		</header>
		<div class="tab_container">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input name='id[]' type='hidden' value='<?php echo ($vo["id"]); ?>'>
				<tr> 
    				...
    				<td>
    					<p>关键词：<?php echo ($vo["keyword"]); ?></p>
    					<p>标&nbsp;&nbsp;&nbsp;&nbsp;题：<?php echo ($vo["title"]); ?></p>
    				</td> 
    				<td>
    					<p>多图文ID：<?php echo ($vo["itemlist"]); ?></p>
    					<p>链接：<?php echo ($vo["url"]); ?></p>
    				</td>
    				<td>
