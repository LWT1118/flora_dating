	...
	<form method='post' action='<?php echo U('posUpdate');?>'>
      ...
      <h3 class="tabs_involved"><?php echo ($location); ?></h3>
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo ($vo["complainter"]["email"]); ?></td>
				<td><a href="<?php echo U('User/edit', array('id'=>$vo['user']['id']));?>"><?php echo ($vo["user"]["email"]); ?></a></td>
				<td><?php echo ($vo["user"]["wechat"]); ?></td>
				<td><?php echo (date('Y-m-d H:i:s',$vo["addtime"])); ?></td>
				<?php if($vo["types"] == showWechatPayFree): ?><td>免费机会</td>
				<?php else: ?><td>付费红线</td><?php endif; ?>
				<td><?php echo ($vo["num"]); ?></td>
				<td><input type="button" r_id="<?php echo ($vo["id"]); ?>" value="同意" status="1" class="alt_btn">&nbsp;<input type="button" r_id="<?php echo ($vo["id"]); ?>" value="拒绝" status="2" class="alt_btn"></td>
...
$(document).ready(function(){
	$('input[status]').click(function(){
		...
		var button = $(this);
		$.getJSON('<?php echo U("/Admin/User/audit");?>', {status:button.attr('status'), rid:button.attr('r_id')}, function(data){
			if(data.err_code == 0){
				alert('操作成功');
