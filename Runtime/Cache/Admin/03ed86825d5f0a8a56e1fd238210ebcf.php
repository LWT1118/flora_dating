...
<section id="main" class="column">
	<article class="module width_full" style="border:none">
		<form method="post" action="<?php echo U('Admin/News/excel','id='.$news['id']);?>">
		<p><button type="submit">导出用户</button></p>
		</form>
	</article>
	<article class="module width_full" style="border:none">
		<form method="post" action="<?php echo U('Admin/News/msg','id='.$news['id']);?>">
	...
	<article class="module width_full">
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
				...
				<td>
				  <p>昵姓：<?php echo (($vo["nickname"])?($vo["nickname"]):'暂未填写'); ?></p>
				  <p>姓名：<?php echo (($vo["realname"])?($vo["realname"]):'暂未填写'); ?></p>
				  <p>手机：<?php echo (($vo["tel"])?($vo["tel"]):'暂未填写'); ?></p>
				</td> 
				<td><?php if($vo["gender"] == 0): ?>男<?php else: ?>女<?php endif; ?></td>
				<td>
