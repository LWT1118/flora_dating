...
<section id="main" class="column">
	<form method='post' action='<?php echo U('menuPosUpdate');?>'>
		<article class="module width_full">
		<header><h3 class="tabs_involved"><?php echo ($location); ?></h3>
		<ul class="tabs">
   			<li><a href="<?php echo U('menuAdd','pid='.$pid);?>"><i class='icon-plus'></i> 添加新条目</a></li>
		</ul>
		</header>
		<div class="tab_container">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
    				...
    				<td><?php echo ($vo["val"]); ?><br><?php echo ($vo["content"]); ?></td>
    				<td>
    					<input style='width:60px !important' name='pos[]' type='number' min="1" max="999999" value='<?php echo ($vo["pos"]); ?>'>
    					<input name='id[]' type='hidden' value='<?php echo ($vo["id"]); ?>'>
    				</td>
    				<td class='text-right'>
    					<?php if(($vo['pid']) == "0"): ?><a class='my-icon' href='<?php echo U('menuAdd','pid='.$vo['id']);?>' title="添加子菜单"><i class='icon-plus'></i></a><?php endif; ?>
    					<a class='my-icon' href='<?php echo U('menuEdit','id='.$vo['id']);?>' title="编辑"><i class='icon-pencil'></i></a>
    					<a class='my-icon' href='javascript:sure("<?php echo U('menuDel','id='.$vo['id']);?>")' title="删除"><i class='icon-trash'></i></a>
    				</td> 
		...
		<footer>
			<div class="submit_link">
				<input onclick="document.location='<?php echo U('menuCreate');?>'" type="button" value="发布自定义菜单" class="alt_btn">
