...
<section id="main" class="column">
	...
	<header><h3 class="tabs_involved"><?php echo ($location); ?></h3>
        <ul class="tabs">
            <li><a href="<?php echo U('add');?>"><i class='icon-plus'></i> 添加投票</a></li>
        </ul>
    </header>
            ...
            <div id="tab1" class="tab_content">
                <table class="tablesorter" cellspacing="0">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ls): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($ls["start_time"]); ?></td>
                            <td><?php echo ($ls["end_time"]); ?></td>
                            <td><?php echo ($ls["status"]); ?></td>
                            <td><?php echo ($ls["content"]); ?></td>
                            <td><img src="<?php echo ($ls["image"]); ?>" style="width:120px;height:80px"/></td>
                            <td>
                                <a class='my-icon' href='<?php echo U('add','id='.$ls['id']);?>' title="编辑"><i class='icon-pencil'></i></a>
                                <a class='my-icon' href='javascript:sure("<?php echo U('voteDelete','id='.$ls['id']);?>")' title="删除"><i class='icon-trash'></i></a>                                
                            </td>
