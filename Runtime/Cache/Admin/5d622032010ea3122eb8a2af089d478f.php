...
<section id="main" class="column">	
	...
	<form method='post' action="__URL__/doadd">
		...
		<div class="module_content">
			<fieldset style="width:24%; float:left; margin-right:1%; margin-top:0">
				<label>投票主图</label>
				<input type='hidden' id='img_input' name='img' value='<?php if(($data)): echo ($data["image"]); endif; ?>'>
				<div class="clear"></div>
				<div img="img_subject" hid="img_input" data-toggle="modal" data-target="#myModal" title='修改图片' style='display:block; width:200px; margin:0 auto; overflow:hidden'>
					<img id="img_subject" src="<?php if(($data)): echo ($data["image"]); else: ?>holder.js/200x200/#f90:#000/text:点击这里上传图片<?php endif; ?>" class='shop_pic_1 img-responsive img-rounded img-thumbnail'>
				</div>
				<label>&nbsp;</label>
				<div class="clear"></div>
			</fieldset>
			...
			<fieldset style='width:37%; margin-top:0; float:left'>
				<label>开始时间</label>
				<input class='datepicker' type='text' name='start_time' value='<?php if(($data)): echo (date("Y-m-d H:i:s",$data["start_time"])); endif; ?>'>
			</fieldset>

			<fieldset style='width:37%; margin-top:0; float:left; margin-left:1%'>
				<label>结束时间</label>
				<input class='datepicker' type='text' name='end_time' value='<?php if(($data)): echo (date("Y-m-d H:i:s",$data["end_time"])); endif; ?>'>
			</fieldset>
											
			<fieldset>
				<label>详细描述</label>
				<textarea style='height:50px' name='content'><?php echo ($data["content"]); ?></textarea>
			</fieldset>
	...
	<article class="module width_full">
		...
		<input type="hidden" name="select_id" value="<?php echo ($sid); ?>">
		<input type="hidden" name="vote_id" value="<?php echo ($vote_id); ?>">
		<input type='hidden' id='hid_select_image' name='img' value='<?php echo ($voteinfo["image"]); ?>'>
		...
		<div class="module_content">
			<table class="tablesorter" cellspacing="0">
				...
				<tbody>				
					<tr>
						<td style="text-align:left"><input type="text" name="order" style="width:80px" value='<?php if($voteinfo): echo ($voteinfo["order"]); endif; ?>'></td>
						<td style="text-align:left"><input type="text" name="title" style="width:250px" value='<?php if($voteinfo): echo ($voteinfo["title"]); endif; ?>'></td>
						...
						<div img="img_select" hid="hid_select_image" data-toggle="modal" data-target="#myModal" title='修改图片' style='display:block; margin:0 auto; overflow:hidden'>
							<img id="img_select" src="<?php if($voteinfo["image"] == ''): ?>holder.js/150x150/#f90:#000/text:点击这里上传图片<?php else: echo ($voteinfo["image"]); endif; ?>" class='shop_pic_1 img-responsive img-rounded img-thumbnail'>
						</div>
						</td>
						<td style="text-align:left"><input type="text" name="add_num" style="width:80px" value='<?php if($voteinfo): echo ($voteinfo["add_num"]); endif; ?>'></td>
						<td style="text-align:left">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ls): $mod = ($i % 2 );++$i;?><tr>
							<td><?php echo ($ls["order"]); ?></td>
							<td><?php echo ($ls["title"]); ?></td>
							<td><img src="<?php echo ($ls["image"]); ?>" style="width:120px;height:80px"/></td>
							<td><?php echo ($ls["add_num"]); ?></td>
							<td>
								<a class='my-icon' href='<?php echo U('add',array('id'=>$ls['vote_id'],'sid'=>$ls['id']));?>' title="编辑"><i class='icon-pencil'></i></a>
								<a class='my-icon' href='javascript:sure("<?php echo U('selectDelete',array('id'=>$ls['vote_id'],'sid'=>$ls['id']));?>")' title="删除"><i class='icon-trash'></i></a>
							</td>
    ...
    <form method='post' enctype='multipart/form-data' id='myForm' action='<?php echo U('upload');?>'>
      ...
      <div class="modal-body">
        	<fieldset>
        		...
        		<div class="clear"></div>
        		<p align='center'>	
				<?php if(strlen($data['img']) > 0): ?><img class='shop_pic_2' src='__UPLOAD__/vote/<?php echo ($data["img"]); ?>' class='img-responsive img-rounded img-thumbnail'>
				<?php else: ?>
