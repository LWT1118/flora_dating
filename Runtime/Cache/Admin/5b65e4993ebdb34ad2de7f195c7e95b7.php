...
<section id="main" class="column">
    <form method='post' action="__URL__/addSelect">
        <article class="module width_full">
            <header><h3><?php echo ($location); ?></h3>
            </header>
            <div class="module_content">
                <fieldset style="width:24%; float:left; margin-right:1%; margin-top:0">
                    <label>选项图片</label>
                    <input type='hidden' id='img_input' name='img' value='<?php if($voteinfo["image"] == ''): else: echo ($voteinfo["image"]); endif; ?>'>
                    <div class="clear"></div>
                    <div data-toggle="modal" data-target="#myModal" title='修改图片' style='display:block; width:200px; margin:0 auto; overflow:hidden'>
                        <?php if($voteinfo["image"] == ''): ?><img src="holder.js/200x200/#f90:#000/text:点击这里上传图片" class='shop_pic_1 img-responsive img-rounded img-thumbnail'>
                        <?php else: ?>
                            <img src="<?php echo ($voteinfo["image"]); ?>" class='shop_pic_1 img-responsive img-rounded img-thumbnail'><?php endif; ?>
                    </div>
                    <label>&nbsp;</label>
                    <div class="clear"></div>
                </fieldset>
                ...
                <fieldset>
                    <label>选项标题</label>
                    <input name='title' type="text" value='<?php if($voteinfo["title"] == ''): else: echo ($voteinfo["title"]); endif; ?>'>
                    <input type="hidden" name="select_id" value="<?php echo ($voteinfo["id"]); ?>">
                </fieldset>

                <fieldset>
                    <label>所属主题</label>
                    <input name='theme' type="text" value='<?php echo ($votes["title"]); ?>' disabled>
                    <input name='vote_id' type="hidden" value='<?php echo ($votes["id"]); ?>'>
                </fieldset>
                <div class="clear"></div>
                ...
                <table class="tablesorter" cellspacing="0">
                    ...
                    <?php else: ?>
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ls): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    <a class='my-icon' href='<?php echo U('index',array('id'=>$votes['id'],'sid'=>$ls['id']));?>' title="编辑"><i class='icon-pencil'></i></a>
                                    <a class='my-icon' href='javascript:sure("<?php echo U('selectDelete',array('id'=>$votes['id'],'sid'=>$ls['id']));?>")' title="删除"><i class='icon-trash'></i></a>
                                </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <fieldset>
                    ...
                    <div class="clear"></div>
                    <p align='center'>
                        <?php if($voteinfo["image"] == ''): ?><img class='shop_pic_2' src="holder.js/360x200/#f90:#000/text:点击下面浏览按钮上传图片" class='img-responsive img-rounded img-thumbnail'>
                        <?php else: ?>
                            <img class='shop_pic_2' src='<?php echo ($voteinfo["image"]); ?>' class='img-responsive img-rounded img-thumbnail'><?php endif; ?>
                    </p>
                </fieldset>
                <fieldset>
                    <label>上传新图</label>
