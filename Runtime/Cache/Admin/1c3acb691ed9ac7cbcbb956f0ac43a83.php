...
<ul class="toggle">
	...
	<section id="main" class="column">
		<form method='post' action='<?php echo U('News/update','action=save');?>'>
...
<fieldset style="width:24%; float:left; margin-right:1%; margin-top:0">
	...
	<div data-toggle="modal" data-target="#myModal" title='修改图片' style='display:block; width:200px; margin:0 auto; overflow:hidden'>
		<?php if(strlen($data['img']) > 0 AND (file_exists('upload/news/thumb200_'.$data['img']))): ?><img src='__UPLOAD__/news/thumb200_<?php echo ($data["img"]); ?>' class='shop_pic_1 img-responsive img-rounded img-thumbnail'>
		<?php else: ?>
			<img src="__PUBLIC__/images/add.gif" class='shop_pic_1 img-responsive img-rounded img-thumbnail'><?php endif; ?>
	</div>
	<label>&nbsp;</label>
	<div class="clear"></div>
</fieldset>
...
<fieldset style='width:37%; margin-top:0; float:left'>
	<label>活动分类</label>
	<select name='cat'>
	<?php if(is_array($catlist)): $i = 0; $__LIST__ = $catlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$catlist): $mod = ($i % 2 );++$i; $var = ' '; ?>
		<?php if($data['cat'] == $catlist['id']): $var = 'selected=selected'; endif; ?>
		<option <?php echo ($var); ?> value='<?php echo ($catlist["id"]); ?>'><?php echo ($catlist["cat"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	</select>
</fieldset>
...
<fieldset style='width:37%; margin-top:0; float:left; margin-left:1%'>
	<label>报名状态</label>
	<select name='alive'>
	<option <?php if(($data['alive']) == "0"): ?>selected='selected'<?php endif; ?> value="0">允许报名</option>
	<option <?php if(($data['alive']) == "1"): ?>selected='selected'<?php endif; ?> value="1">允许男性报名</option>
	<option <?php if(($data['alive']) == "2"): ?>selected='selected'<?php endif; ?> value="2">允许女性报名</option>
	<option <?php if(($data['alive']) == "3"): ?>selected='selected'<?php endif; ?> value="3">报名结束</option>
	</select>
</fieldset>

<fieldset style='width:37%; margin-top:0; float:left'>
	<label>开始时间</label>
	<input class='datepicker' type='text' name='start_time' value='<?php echo (date('Y-m-d H:i',$data["start_time"])); ?>'>
</fieldset>

<fieldset style='width:37%; margin-top:0; float:left; margin-left:1%'>
	<label>结束时间</label>
	<input class='datepicker' type='text' name='end_time' value='<?php echo (date('Y-m-d H:i',$data["end_time"])); ?>'>
</fieldset>

<fieldset>
	<label>活动地址</label>
	<textarea style='height:43px' name='summary'><?php echo ($data["summary"]); ?></textarea>
</fieldset>

<fieldset>
	<label>活动详细</label>
	<textarea style='height:200px' name='content'><?php echo ($data["content"]); ?></textarea>
</fieldset>
<div class="clear"></div>
				</div>
			<footer>
    ...
    <form method='post' enctype='multipart/form-data' id='myForm' action='<?php echo U('upload');?>'>
      ...
      <div class="modal-body">
        	<fieldset>
        		...
        		<div class="clear"></div>
        		<p align='center'>	
				<?php if(strlen($data['img']) > 0): ?><img class='shop_pic_2' src='__UPLOAD__/news/thumb300_<?php echo ($data["img"]); ?>' class='img-responsive img-rounded img-thumbnail'>
				<?php else: ?>
					<img class='shop_pic_2' src="holder.js/200x200/#f90/text:选择电脑中的图片" class='img-responsive img-rounded img-thumbnail'><?php endif; ?>
				</p>
			</fieldset>
        	<fieldset>
        		<label>上传新图</label>
...
<link type="text/css" rel="stylesheet" href="__HOMEPUBLIC__/css/dhtmlgoodies_calendar/dhtmlgoodies_calendar.css?random=20051112" />
<script type="text/javascript" src="__HOMEPUBLIC__/js/dhtmlgoodies_calendar/dhtmlgoodies_calendar.js?random=20060118"></script>
