...
<ul class="toggle">
	...
	<section id="main" class="column">
		<form method='post' action='<?php echo U('News/update','action=add');?>'>
...
<fieldset style='width:37%; margin-top:0; float:left'>
	<label>开始时间</label>
	<input class='datepicker' type='text' name='start_time' value='<?php echo ($data["start_time"]); ?>'>
</fieldset>

<fieldset style='width:37%; margin-top:0; float:left; margin-left:1%'>
	<label>结束时间</label>
	<input class='datepicker' type='text' name='end_time' value='<?php echo ($data["end_time"]); ?>'>
</fieldset>
...
<fieldset>
	<label>活动分类</label>
	<select name='cat'>
	<?php if(is_array($catlist)): $i = 0; $__LIST__ = $catlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$catlist): $mod = ($i % 2 );++$i;?><option value='<?php echo ($catlist["id"]); ?>'><?php echo ($catlist["cat"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	</select>
</fieldset>						
