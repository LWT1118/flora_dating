		...
		<div class="tab_container">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input name='id[]' type='hidden' value='<?php echo ($vo["id"]); ?>'>
				<tr> 
   					...
   					<?php if(strlen($vo['img']) > 0): ?><img src='__UPLOAD__/news/thumb_<?php echo ($vo["img"]); ?>' class='img-responsive img-rounded img-thumbnail'>
   					<?php else: ?>
   						<img src="__HOMEPUBLIC__/images/no_pic.gif" class='img-responsive img-rounded img-thumbnail'><?php endif; ?>
   					</div>
					</td> 
    				<td>
    					<p>活动：<?php echo ($vo["title"]); ?></p>
    					<p>地址：<?php echo ($vo["summary"]); ?></p>
              <p>分类：<a href='<?php echo U('index','cat='.$vo['cat']);?>'><?php echo ($vo["cat_name"]); ?></a></p>
              <p>发起：<?php echo (($vo["realname"])?($vo["realname"]):'管理员'); ?></p>
			  <p>报名状态：<?php switch($vo['alive']): case "0": ?>允许报名<?php break; case "1": ?>允许男性报名<?php break; case "2": ?>允许女性报名<?php break; case "3": ?>报名结束<?php break; endswitch;?></p>
    				</td> 
            <td>
              <p><?php echo (date('Y-m-d H:i',$vo["start_time"])); ?></p>
              <p><?php echo (date('Y-m-d H:i',$vo["end_time"])); ?></p>
            </td>
    				<td>
    					<p>报名：<a href="<?php echo U('regList',array('newsid'=>$vo['id']));?>"><?php echo ($vo["reg"]); ?> 人</a></p>
    					<p>签到：<a href="<?php echo U('arrList',array('newsid'=>$vo['id']));?>"><?php echo ($vo["arrival"]); ?> 人</a></p>
    				</td>
    				<td>
              
              <p><input style='width:70px !important' name='pos[]' type='number' min="1" max="999999" value='<?php echo ($vo["pos"]); ?>'></p>
            </td>
    				<td>
    					<?php if(($vo['status']) == "0"): ?><a class='btn btn-xs btn-success' href='<?php echo U('statusUpdate','id='.$vo['id'].'&status='.$vo['status']);?>' title="活动审核">批准</a>
              <?php else: ?>
                <a class='btn btn-xs btn-danger' href='<?php echo U('statusUpdate','id='.$vo['id'].'&status='.$vo['status']);?>' title="活动审核">拒绝</a><?php endif; ?>
    				</td>
    				<td>
    					
    					<a target='_blank' class='my-icon' href='<?php echo U('qrcode','id='.$vo['id']);?>' title="生成二维码"><i class='icon-qrcode'></i></a>
