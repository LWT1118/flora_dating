		...
		<div class="row">
			...
			<div class="bony-h"></div>
			  ...
			  <ul class="list-group">				  
				  ...
				  <?php if($data['status'] == 0): ?><input type="radio" name="option" value="<?php echo ($option["id"]); ?>" />
				  <?php else: ?>
				  <input type="checkbox" name="option" value="<?php echo ($option["id"]); ?>" /><?php endif; ?>				  
				  ...
				  <?php else: ?>
					  <?php if($voted > 0): ?><div class="progress" style="margin-top:5px">
...
$(document).ready(function(){
	...
	$('#showBase').on('hide.bs.modal', function (e) {	
		if($('#info-container').text() == '投票成功'){
			location.href = '/Mobile/Vote/result/?id=<?php echo ($data["id"]); ?>';
		}
	});
});
</script>
